...
class UserService {
  constructor() {
    ...
    this.statusRefreshIntervalMs = 2 * 60 * 1000;
  ...
  startStatusRefreshTimer() {
    ...
    log(`Starting online status refresh every ${this.statusRefreshIntervalMs / 60000} minutes`);
    
    this.statusRefreshTimer = setInterval(() => {
      this.refreshAllOnlineStatuses().catch(err => {
        logError('Failed to refresh online statuses:', err);
      });
  ...
  async refreshAllOnlineStatuses() {
    ...
    log(`Refreshing online status for ${otherUserIds.length} cached users`);
    ...
    try {
      ...
      for (const status of statuses) {
        ...
        if (cachedProfile) {
          ...
          if (wasOnline !== status.online) updatedCount++;
        }
      }
      
      log(`Online status refresh complete. ${updatedCount} users changed status.`);
      window.dispatchEvent(new CustomEvent('online-status-updated'));
    } catch (error) {
      logError('Failed to refresh online statuses:', error);
    }
  }
  ...
  async refreshOnlineStatusForUsers(userIds) {
    ...
    try {
      ...
      logError('Failed to refresh online status for specific users:', error);
    }
  }
